#!/usr/bin/env python3
# ABOUTME: Tests PDF size and count limits on Bedrock via the invoke_model API.
...
def send_pdfs(client, model_id, pdf_list):
    """Send PDFs via invoke_model with 1M context beta. Returns (succeeded, detail)."""
    content = []
...
def main():
    ...
    for desc, pdf_list in tests:
        ...
        print(f"  {desc}")
        print(
            f"  raw: {total_mb:.1f} MB, encoded: ~{encoded_mb:.1f} MB, {len(pdf_list)} PDF(s)"
        )
        print("=" * 70)
