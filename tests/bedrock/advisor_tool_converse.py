#!/usr/bin/env python3
# ABOUTME: Tests the advisor_20260301 tool on Amazon Bedrock via the Converse API.
# ABOUTME: Validates that a faster executor model can consult a higher-intelligence advisor model mid-generation.

"""
Advisor Tool Test for Amazon Bedrock (Converse API)
=====================================================
...
Advisor tool configuration and beta headers are passed via
...
1. Basic advisor call — send a complex prompt that should trigger the executor
   to consult the advisor, verify response contains text output.

Requirements:
    uv add boto3

Usage:
    uv run python tests/bedrock/advisor_tool_converse.py
"""
...
def test_basic_advisor(client, model_id):
    """Test basic advisor tool call via Converse. Returns (status, error_msg)."""
    print("=" * 70)
    ...
    try:
        ...
        if has_text or has_tool_use:
            print("\n  Result: PASS - got response with advisor tool configured")
            return ("PASS", None)
        else:
