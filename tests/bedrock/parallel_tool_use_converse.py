#!/usr/bin/env python3
# ABOUTME: Tests parallel tool use on Amazon Bedrock via the Converse API.
# ABOUTME: Verifies that Claude returns multiple toolUse blocks in a single response.

"""
Parallel Tool Use Test for Amazon Bedrock (Converse API)
=========================================================

Tests that Claude can make multiple tool calls in a single response via
the Converse API. Presents a prompt that naturally requires parallel
lookups (weather + time in multiple cities) and validates:

1. Multiple toolUse blocks in one response (parallel calls)
...
Usage:
    uv run python tests/bedrock/parallel_tool_use_converse.py
"""
...
def simulate_tool_result(tool_use):
    ...
    if name == "get_weather":
        ...
        timezone = str(tool_input.get("timezone", ""))
...
def test_parallel_tool_use(client, model_id):
    ...
    messages = [
        {
            "role": "user",
            "content": [
                {"text": "What's the weather in SF and NYC, and what time is it there?"}
            ],
        }
    ]

    # Request 1: expect multiple toolUse blocks
    ...
    output2 = response2.get("output", {}).get("message", {})
    content2 = output2.get("content", [])
    stop_reason2 = response2.get("stopReason")
    text_blocks = [b["text"] for b in content2 if isinstance(b, dict) and "text" in b]

    print(f"\n  stopReason: {stop_reason2}")
    if text_blocks:
        print(f"  Response: {text_blocks[0][:200]}")

    if stop_reason2 != "end_turn":
        msg = f"Expected stopReason='end_turn' for final response, got '{stop_reason2}'"
        print(f"\n  Result: FAIL - {msg}")
        return ("FAIL", msg)
