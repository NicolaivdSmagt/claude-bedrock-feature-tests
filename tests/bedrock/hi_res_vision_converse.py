#!/usr/bin/env python3
# ABOUTME: Tests high-resolution vision support on Amazon Bedrock via the Converse API.
# ABOUTME: Opus 4.7 processes images up to 2576px (up from 1568px on previous models).

"""
High-Resolution Vision Test for Amazon Bedrock (Converse API)
===============================================================

Tests Opus 4.7's enhanced vision capability via the Converse API. Images are
processed at up to 2576px (up from 1568px on prior models). Higher resolution
means ~3x more image tokens. No API changes are needed — the resolution is
applied server-side.

This test sends a high-resolution image and verifies:
- The request succeeds
- The image token usage is higher than a low-res baseline
...
Usage:
    uv run python tests/bedrock/hi_res_vision_converse.py
"""
...
def read_image_bytes(image_name):
    """Read image bytes from the files/ directory."""
...
def send_image(client, model_id, image_name, prompt):
    """Send an image via Converse. Returns (response, error)."""
    image_bytes = read_image_bytes(image_name)
    ...
    print("\n--- REQUEST (image bytes omitted for brevity) ---")
    req_preview = {
                ...
                "content": [
                    {
                        "image": {
                            "format": "jpeg",
                            "source": {"bytes": f"<{len(image_bytes)} bytes>"},
                        }
                    },
                    {"text": prompt},
                ],
    ...
    print(json.dumps(req_preview, indent=2))
...
def test_hi_res_image(client, model_id):
    """Test sending a high-resolution image. Returns (status, error_msg, usage)."""
    print("=" * 70)
    print(f"TEST: HIGH-RESOLUTION IMAGE ({HI_RES_IMAGE})")
    print("=" * 70)

    response, error = send_image(
        client,
        model_id,
        HI_RES_IMAGE,
...
def test_token_comparison(hi_res_usage, low_res_usage):
    ...
    hi_tokens = hi_res_usage.get("inputTokens", 0)
    low_tokens = low_res_usage.get("inputTokens", 0)

    print(f"\n  Hi-res image inputTokens: {hi_tokens}")
    print(f"  Low-res image inputTokens: {low_tokens}")
