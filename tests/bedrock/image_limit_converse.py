#!/usr/bin/env python3
# ABOUTME: Tests the maximum image count limit on Bedrock via the Converse API.
...
def send_images(client, model_id, count):
    """Send `count` images via Converse. Returns (succeeded: bool, detail: str)."""
    ...
    try:
        ...
        n_blocks = len(output_msg.get("content", []))
        detail = (
...
def main():
    ...
    for count in ACCEPTED_COUNTS + REJECTED_COUNTS:
        print("=" * 70)
        print(f"  Sending {count} images ({len(IMAGE_BYTES)} bytes each)...")
