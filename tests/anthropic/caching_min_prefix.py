#!/usr/bin/env python3
# ABOUTME: Tests minimum prefix size required for prompt caching on the Anthropic 1st-party API.
...
def run_cache_pair_anthropic(client, model: str, system_prefix: str, label: str):
    """Send two Anthropic API requests with the same system prefix and check caching.
    ...
    cache_creation = getattr(usage1, "cache_creation_input_tokens", 0) or 0
    cache_read_r1 = getattr(usage1, "cache_read_input_tokens", 0) or 0
    ...
    try:
        response2 = client.messages.create(
            ...
            messages=[
                {
                    "role": "user",
                    "content": "What is the difference between Paxos and Raft?",
                }
            ],
        )
    except Exception as e:
    ...
    cache_creation_r2 = getattr(usage2, "cache_creation_input_tokens", 0) or 0
    ...
    return {
        ...
        "input_tokens_r1": usage1.input_tokens or 0,
    }
...
def main():
    ...
    # --- Anthropic: below threshold (~1940 tokens) ---
    anthropic_below = run_cache_pair_anthropic(
        client,
        model_id,
        prefix_below,
        "Anthropic API - below threshold (~1940 tokens)",
    )

    # --- Anthropic: above threshold (~2051 tokens) ---
    anthropic_above = run_cache_pair_anthropic(
        client,
        model_id,
        prefix_above,
        "Anthropic API - above threshold (~2051 tokens)",
    )
    ...
    if anthropic_below is None:
        print("FAIL: Below-threshold test encountered an error")
        all_passed = False
    else:
        total_tokens = (
            anthropic_below["input_tokens_r1"] + anthropic_below["cache_creation"]
        )
        print(f"Below threshold (~1940 target, observed ~{total_tokens} tokens):")
        if (
            anthropic_below["cache_creation"] == 0
            and anthropic_below["cache_read"] == 0
        ):
            print("  PASS: No caching occurred (prefix below threshold)")
        else:
            print(
                f"  INFO: Caching WAS observed (cache_creation={anthropic_below['cache_creation']}, "
                f"cache_read={anthropic_below['cache_read']})"
            )

    if anthropic_above is None:
        print("FAIL: Above-threshold test encountered an error")
        all_passed = False
    else:
        total_tokens = (
            anthropic_above["input_tokens_r1"] + anthropic_above["cache_creation"]
        )
        print(f"Above threshold (~2051 target, observed ~{total_tokens} tokens):")

        if anthropic_above["cache_creation"] > 0:
            print(
                f"  PASS: Cache created with {anthropic_above['cache_creation']} tokens"
            )
        else:
            print("  FAIL: No cache creation on first request")
            all_passed = False

        if anthropic_above["cache_read"] > 0:
            print(f"  PASS: Cache read with {anthropic_above['cache_read']} tokens")
        else:
            print("  FAIL: No cache read on second request")
            all_passed = False

        if anthropic_above["cache_creation_r2"] == 0:
            print("  PASS: Second request did not create a new cache entry")
        else:
            print(
                f"  WARN: Second request created cache with "
                f"{anthropic_above['cache_creation_r2']} tokens"
            )
